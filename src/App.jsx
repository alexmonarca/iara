...
import { 
  ...
  ExternalLink, Lock, HelpCircle, History, Instagram, Wifi
} from 'lucide-react';

// ==========================================
// ⚠️ CONFIGURAÇÃO OBRIGATÓRIA
// Preencha isso antes de subir para o Vercel!
// ==========================================
const SUPABASE_URL = "https://wjyrinydwrazuzjczhbw.supabase.co"; // Cole sua URL aqui (ex: https://xyz.supabase.co)
...
const InputGroup = ({ label, type = "text", placeholder, value, onChange, multiline = false, helpText, required = false }) => (
  <div className="mb-5">
    ...
    {multiline ? (
      <textarea className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-gray-100 focus:ring-2 focus:ring-orange-500/30 focus:border-orange-500/50 outline-none transition-all min-h-[100px] text-sm leading-relaxed" placeholder={placeholder} value={value || ''} onChange={onChange} required={required} />
    ) : (
      <input type={type} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-gray-100 focus:ring-2 focus:ring-orange-500/30 focus:border-orange-500/50 outline-none transition-all text-sm" placeholder={placeholder} value={value || ''} onChange={onChange} required={required} />
    )}
  ...
  return ( 
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4"> 
      <div className="max-w-md w-full bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-2xl"> 
        <div className="text-center mb-8"> <BrainCircuit className="w-12 h-12 text-orange-400 mx-auto mb-4" /> <h1 className="text-2xl font-bold text-white">IARA Gym</h1> <p className="text-gray-400 mt-2">{isSignUp ? 'Crie sua conta e configure sua IA' : 'Acesse o Painel da sua Academia'}</p> </div> 
...
export default function App() {
  ...
  if (!isLibLoaded) return <div className="min-h-screen bg-gray-950 flex items-center justify-center flex-col text-orange-400"><RefreshCw className="w-10 h-10 animate-spin mb-4" /><p>Conectando ao servidor...</p></div>;
...
function Dashboard({ session }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 1. Carregar Configs
        const { data, error } = await supabaseClient.from('gym_configs').select('*').eq('user_id', userId).maybeSingle();
        if (data) {
           ...
           if (data.extra_channels_count) setExtraChannels(data.extra_channels_count);
             ...
             // Criar linha inicial se não existir
             await supabaseClient.from('gym_configs').insert({ user_id: userId, email: session.user.email });
        }
  ...
  const handleSave = async () => {
    ...
    const dataToSave = {
        user_id: userId,
        gym_name: gymData.gym_name, phone: gymData.phone, pix_key: gymData.pix_key,
        address: gymData.address, branches: gymData.branches, opening_hours: gymData.opening_hours,
        pricing_info: gymData.pricing_info, faq_text: gymData.faq_text, tone_of_voice: gymData.tone_of_voice,
        observations: gymData.observations, allow_calls: gymData.allow_calls, reply_groups: gymData.reply_groups,
        reply_audio: gymData.reply_audio, send_images: gymData.send_images, integrate_agenda: gymData.integrate_agenda,
        recognize_payments: gymData.recognize_payments, connection_status: connectionStep, updated_at: new Date(),
        needs_reprocessing: true, extra_channels_count: extraChannels
    };
    try {
      const { error } = await supabaseClient.from('gym_configs').upsert([dataToSave], { onConflict: 'user_id' });
      if (error) throw error;
      alert("Configurações salvas! A IA será atualizada em instantes.");
    } catch (error) { alert("Erro ao salvar: " + error.message); } finally { setIsSaving(false); }
  };
  ...
  const totalPrice = 250 + (gymData.branches.length * 150) + (gymData.omnichannel ? 25 : 0) + (gymData.integrate_agenda ? 50 : 0) + (gymData.recognize_payments ? 50 : 0) + (extraChannels * 50);
        ...
        return (
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-gradient-to-br from-orange-500/10 to-orange-900/10 border border-orange-500/20 rounded-xl p-6 text-white shadow-lg">
                <div className="flex justify-between items-start">
                  <div><p className="text-orange-200 text-sm font-medium">Status da IA</p><h2 className="text-3xl font-bold mt-1 text-orange-50">{connectionStep === 'connected' ? 'Ativa' : 'Inativa'}</h2></div>
                  <div className="bg-orange-500/10 p-2 rounded-lg"><BrainCircuit className="w-6 h-6 text-orange-400" /></div>
                </div>
              </div>
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
                ...
                <div className="mt-4 w-full bg-gray-700 rounded-full h-2 overflow-hidden"><div className="bg-orange-500 h-2 rounded-full" style={{ width: `${(trialDaysLeft/7)*100}%` }}></div></div>
                <p className="text-right text-xs text-orange-400 mt-1 font-bold">{trialDaysLeft} dias restantes</p>
              </div>
            </div>
                      ...
                      <div key={index} className={`flex ${log.sender === 'ai' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] p-3 rounded-2xl ${log.sender === 'ai' ? 'bg-orange-500/20 border border-orange-500/30 text-orange-100 rounded-br-none' : 'bg-gray-700 border border-gray-600 text-gray-200 rounded-bl-none'}`}>
                          <div className="flex items-center gap-2 mb-1 opacity-70 text-[10px] uppercase tracking-wider">
                             {log.sender === 'ai' ? <span>🤖 IARA Gym</span> : <span>👤 Aluno {log.customer_phone}</span>}<span>• {new Date(log.created_at).toLocaleTimeString()}</span>
                          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card title="Cérebro da Operação" icon={BrainCircuit}>
                 ...
                 <div className="my-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <CheckboxGroup icon={PhoneCall} label="Aceitar ligações WhatsApp?" subLabel="aceita mas não atende." checked={gymData.allow_calls} onChange={(val) => setGymData({...gymData, allow_calls: val})} />
          ...
          <div className="max-w-4xl mx-auto animate-in fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8">
                 <h3 className="text-xl font-semibold text-white mb-6">Canais Extras</h3>
                 <div className="bg-gray-900 p-4 rounded-xl border border-gray-700 mb-4">
                    ...
                    {extraChannels > 0 && <div className="bg-pink-500/10 border border-pink-500/20 p-3 rounded text-xs text-pink-200"><p className="font-bold mb-1 flex items-center gap-1"><Instagram className="w-3 h-3" /> Instagram Ativo</p>IA reposta stories e atende direct.</div>}
                 </div>
              </div>
            </div>
          </div>
        ...
        return (
           <div className="max-w-2xl mx-auto text-center pt-10 animate-in fade-in">
             <div className="relative group w-24 h-24 mx-auto mb-6">
                {gymData.logo_url ? ( <img src={gymData.logo_url} alt="Logo" className="w-full h-full rounded-full object-cover border-2 border-orange-500" /> ) : ( <div className="w-full h-full bg-gray-800 rounded-full flex items-center justify-center border-2 border-dashed border-gray-600 group-hover:border-orange-500 transition-colors cursor-pointer"><Upload className="w-8 h-8 text-gray-500 group-hover:text-orange-500" /></div> )}
             ...
             <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 text-left mb-8 shadow-lg">
                ...
                <div className="space-y-4">
                   ...
                   <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded text-xs text-blue-200 flex gap-2 items-start"><ShieldAlert className="w-4 h-4 mt-0.5 flex-shrink-0" /><div><span className="font-bold block mb-1">Atenção:</span>Mudanças aqui não refletem na conta de pagamentos (MonarcaHub.com).</div></div>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans flex flex-col md:flex-row">
      <aside className="hidden md:flex w-64 flex-col bg-gray-900 border-r border-gray-800 h-screen sticky top-0">
        <div className="p-6 flex items-center gap-2 font-bold text-2xl text-orange-400 tracking-tighter"><BrainCircuit className="w-8 h-8" /> IARA Gym</div>
      ...
      <div className="md:hidden bg-gray-900 p-4 border-b border-gray-800 flex justify-between items-center sticky top-0 z-10">
        <div className="flex items-center gap-2 font-bold text-xl text-orange-400"><BrainCircuit className="w-8 h-8" /> IARA Gym</div><button className="text-gray-300"><Settings /></button>
      </div>
